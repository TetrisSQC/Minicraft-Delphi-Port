...
{$IFDEF MACOS}
{$IFDEF IOS}
  bassdll = 'redist\libbass.a';
{$ELSE}
  bassdll = 'libbass.dylib';
{$ENDIF}
{$ENDIF}
{$IFDEF ANDROID}
  bassdll = 'redist\libbass.so';
{$ENDIF}
...
function BASS_RecordGetInputName(input: LongInt): PByteChar; {$IFDEF MSWINDOWS} stdcall{$ELSE} cdecl{$ENDIF}; external bassdll;
...
{$IFDEF MSWINDOWS}
  BASS_Init: function(device: LongInt; freq, flags: Cardinal; win: HWND;
    clsid: PGUID): BOOL; stdcall;
{$ELSE}
  BASS_Init: function(device: LongInt; freq, flags: Cardinal; win: Pointer;
    clsid: Pointer): BOOL; cdecl;
{$ENDIF}
...
{$ENDIF}
  ...
  BASS_StreamCreateFileUser: function(system, flags: Cardinal;
    var procs: BASS_FILEPROCS; user: Pointer): HSTREAM; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
...
{$ENDIF}
  ...
  BASS_StreamCreateFileUser := GetProcAddress(FBassDLL,
    'BASS_StreamCreateFileUser');
  BASS_StreamFree := GetProcAddress(FBassDLL, 'BASS_StreamFree');
  BASS_StreamGetFilePosition := GetProcAddress(FBassDLL,
  ...
  BASS_RecordGetDeviceInfo := GetProcAddress(FBassDLL,
  ...
  BASS_RecordGetInputName := GetProcAddress(FBassDLL,
  ...
  BASS_ChannelBytes2Seconds := GetProcAddress(FBassDLL,
    'BASS_ChannelBytes2Seconds');
  BASS_ChannelSeconds2Bytes := GetProcAddress(FBassDLL,
  ...
  BASS_ChannelSetAttribute := GetProcAddress(FBassDLL,
    'BASS_ChannelSetAttribute');
  BASS_ChannelGetAttribute := GetProcAddress(FBassDLL,
    'BASS_ChannelGetAttribute');
  BASS_ChannelSlideAttribute := GetProcAddress(FBassDLL,
    'BASS_ChannelSlideAttribute');
  BASS_ChannelIsSliding := GetProcAddress(FBassDLL, 'BASS_ChannelIsSliding');
  BASS_ChannelSet3DAttributes := GetProcAddress(FBassDLL,
    'BASS_ChannelSet3DAttributes');
  BASS_ChannelGet3DAttributes := GetProcAddress(FBassDLL,
    'BASS_ChannelGet3DAttributes');
  BASS_ChannelSet3DPosition := GetProcAddress(FBassDLL,
    'BASS_ChannelSet3DPosition');
  BASS_ChannelGet3DPosition := GetProcAddress(FBassDLL,
    'BASS_ChannelGet3DPosition');
  BASS_ChannelGetLength := GetProcAddress(FBassDLL, 'BASS_ChannelGetLength');
  BASS_ChannelSetPosition := GetProcAddress(FBassDLL,
    'BASS_ChannelSetPosition');
  BASS_ChannelGetPosition := GetProcAddress(FBassDLL,
...
begin
  case (env) of
    ...
    EAX_ENVIRONMENT_BATHROOM:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_BATHROOM, 0.653,
        1.499, 0.166);
    EAX_ENVIRONMENT_LIVINGROOM:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_LIVINGROOM, 0.208,
        0.478, 0);
    EAX_ENVIRONMENT_STONEROOM:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_STONEROOM, 0.5,
        2.309, 0.888);
    EAX_ENVIRONMENT_AUDITORIUM:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_AUDITORIUM, 0.403,
        4.279, 0.5);
    EAX_ENVIRONMENT_CONCERTHALL:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_CONCERTHALL, 0.5,
        3.961, 0.5);
    EAX_ENVIRONMENT_CAVE:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_CAVE, 0.5, 2.886, 1.304);
    EAX_ENVIRONMENT_ARENA:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_ARENA, 0.361,
        7.284, 0.332);
    EAX_ENVIRONMENT_HANGAR:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_HANGAR, 0.5, 10.0, 0.3);
    EAX_ENVIRONMENT_CARPETEDHALLWAY:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_CARPETEDHALLWAY, 0.153,
        0.259, 2.0);
    EAX_ENVIRONMENT_HALLWAY:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_HALLWAY, 0.361, 1.493, 0);
    EAX_ENVIRONMENT_STONECORRIDOR:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_STONECORRIDOR, 0.444,
        2.697, 0.638);
    EAX_ENVIRONMENT_ALLEY:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_ALLEY, 0.25,
        1.752, 0.776);
    EAX_ENVIRONMENT_FOREST:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_FOREST, 0.111,
        3.145, 0.472);
    EAX_ENVIRONMENT_CITY:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_CITY, 0.111,
        2.767, 0.224);
    EAX_ENVIRONMENT_MOUNTAINS:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_MOUNTAINS, 0.194,
        7.841, 0.472);
    EAX_ENVIRONMENT_QUARRY:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_QUARRY, 1, 1.499, 0.5);
    EAX_ENVIRONMENT_PLAIN:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_PLAIN, 0.097,
        2.767, 0.224);
    EAX_ENVIRONMENT_PARKINGLOT:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_PARKINGLOT, 0.208,
        1.652, 1.5);
    EAX_ENVIRONMENT_SEWERPIPE:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_SEWERPIPE, 0.652,
        2.886, 0.25);
    EAX_ENVIRONMENT_UNDERWATER:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_UNDERWATER, 1, 1.499, 0);
    EAX_ENVIRONMENT_DRUGGED:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_DRUGGED, 0.875,
        8.392, 1.388);
    EAX_ENVIRONMENT_DIZZY:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_DIZZY, 0.139,
        17.234, 0.666);
    EAX_ENVIRONMENT_PSYCHOTIC:
      result := BASS_SetEAXParameters(EAX_ENVIRONMENT_PSYCHOTIC, 0.486,
        7.563, 0.806);
  else
    result := FALSE;
  end;
